...
export class FanvueBrowser extends BaseBrowser {
  ...
  public async login(setting: IAccountSettings): Promise<IAccountID | undefined> {
    try {
      ...
      const loginPromise = this.page.waitForResponse(response => {
        return response.url().includes("https://www.fanvue.com/api/auth/callback/credentials") && response.request().method() === "POST"
      }, { timeout: 180000 });
      const profilePromise = this.page.waitForResponse(response => {
        return response.url().includes("https://www.fanvue.com/trpc/user.getOwnProfile") && response.request().method() === "GET"
      }, { timeout: 180000 });
  ...
  public async getMonthlyEarnings(): Promise<number> {
    try {
      ...
      const revenue = items.filter(item => new Date(item.paid_at) >= lastMonth).reduce((sum, item) => sum += item.creator_net, 0);
      return revenue / 100
    } catch (error: any) {
      if (error instanceof BotError)
        throw error;
  ...
  public async getFolders(): Promise<string[]> {
    try {
      const resp = await this.page.request.get("https://www.fanvue.com/trpc/vault.getVaultFolders", {
        headers: this.headers,
        params: { input: JSON.stringify({ "json": null, "meta": { "values": ["undefined"] } }) }
      });
      if (!resp.ok()) {
  ...
  public async findVaultInFolder(folderName: string, vaultId: string): Promise<string | undefined> {
    try {
      ...
      while (true) {
        ...
        if (vault)
          break;
        if (!respData.result?.data?.json?.nextCursor)
          break;
  ...
  public async moveVaultToFolder(vaultId: string, folderName: string): Promise<void> {
    try {
      const resp = await this.page.request.post("https://www.fanvue.com/trpc/vault.moveMediaToFolder", {
        headers: this.headers,
        data: { "json": { "sourceFolderName": null, "targetFolderName": folderName, "mediaUuids": [vaultId] }, "meta": { "values": { "sourceFolderName": ["undefined"] } } }
      })
      if (!resp.ok()) {
        throw new BotError("move vault failed", {
          ...
          params: { "json": { "sourceFolderName": null, "targetFolderName": folderName, "mediaUuids": [vaultId] }, "meta": { "values": { "sourceFolderName": ["undefined"] } } },
          status: resp.statusText(),
          response: await resp.text(),
        })
      }
  ...
  public async uploadVault(mediaPath: string): Promise<string> {
    try {
      ...
      const createUploadPromise = this.page.waitForResponse(response => {
        return response.url().includes("https://www.fanvue.com/trpc/media.createMediaMultipartUpload") && response.request().method() === "POST"
      }, { timeout: 600000 });
      const finalUploadPromise = this.page.waitForResponse(response => {
        return response.url().includes("https://www.fanvue.com/trpc/media.finaliseMedia") && response.request().method() === "POST"
      }, { timeout: 600000 });
      await this.page.locator("input#files").first().setInputFiles(mediaPath);
      const [createUploadResp, finalUploadResp] = await Promise.all([createUploadPromise, finalUploadPromise]);
      ...
      if (!createUploadResp.ok() || !finalUploadResp.ok())
        throw new BotError("upload vault failed", {
          ...
          method: "POST",
          endpoint: "https://www.fanvue.com/trpc/media.createMediaMultipartUpload",
      ...
      switch (postType) {
        case PostType.FANS:
          params = {
            ...
            "meta": {
              "values": { "expires_at": ["undefined"], "media_preview_uuid": ["undefined"], "price": ["undefined"] }
            }
          }
          break;
        case PostType.PAID:
          if (!price || price < 3) throw new BotError("schedule post failed", {
  ...
  public async createPost(title: string, mediaId: string) {
    try {
      const params = {
        ...
        "meta": {
          "values": { "expires_at": ["undefined"], "media_preview_uuid": ["undefined"], "price": ["undefined"], "publish_at": ["undefined"] }
        }
      }
      const resp = await this.page.request.post("https://www.fanvue.com/trpc/post.createPost", {
        headers: this.headers,
      ...
      if (!resp.ok()) throw new BotError("create post failed", {
  ...
  public async getUnreadChats(): Promise<IChatMessage[]> {
    try {
      ...
      return chats.map(chat => ({ user: chat.displayName, time: new Date(chat.lastMessage.sentAt), message: chat.lastMessage.text || "[image]" }));
    } catch (error: any) {
      if (error instanceof BotError)
        throw error;
      throw new BotError("get chats failed", {
